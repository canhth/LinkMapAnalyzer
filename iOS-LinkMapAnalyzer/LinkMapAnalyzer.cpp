 ...
 */
LinkMapSessionType
LinkMapAnalyzer::checkLinkMapSession(const char *buf, const LinkMapSessionType preType) {
	if (buf[0] != '#')
	...
	if (strstr(buf, "# Object") != NULL)
		return linkMapObjectFiles;
	if (strstr(buf, "# Sections") != NULL)
		return linkMapSections;
	if (strstr(buf, "# Symbols") != NULL)
		return linkMapSymbols;
	if (strstr(buf, "# Address") != NULL) {
		if (preType == linkMapSections)
			return linkMapSectionAddress;

		if (preType == linkMapSymbols)
			return linkMapSymbolAddress;
	}
	return linkMapUndefine;
}
...
void LinkMapAnalyzer::output() {
	...
	sort(
		objectModules.begin(),
		objectModules.end(),
		[](const ObjectModule *a, const ObjectModule *b) { return a->moduleSize > b->moduleSize; });

	totalSize = 0;
	for_each(objectModules.begin(), objectModules.end(), [&](const ObjectModule *module) {
		formatoutput(module->moduleName, module->moduleSize * 1.0 / 1024, "KB");
 ...
 *  格式化 输出
 */
void LinkMapAnalyzer::formatoutput(const string name, double size, const string sizeUnit) {
...
void LinkMapAnalyzer::ckeckUnkownObjectFile() {
	for_each(objectFiles.begin(), objectFiles.end(), [&](const ObjectFile *file) {
		if (file->fileName == _unkown_file_name || file->belongModule == _unkown_module_name) {
			cout << "[debug]: " << file->fileNo << "-" << file->fileName << "-"
				 << file->belongModule << endl;
		}
	});
}
